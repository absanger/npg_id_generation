...
def concatenate_tags(tags: list[str]):
    """Returns a concatenated list of tags.

    A helper method that converts a list of tags into a single
    string, which can be used to set the `tags` attribute of
    the PacBioEntity class instance.
...
class PacBioEntity(BaseModel):
    """A class that wraps together PacBio data product's attributes.

    This class wraps together the attributes that describe an experiment
    performed on a sequencer by Pacific Biosciences (PacBio). A single
    experiment produces a number of data products. This class models
    a single data product.

    The class provides an instance method for generating a unique ID for
    the single product this class models.

    The class inherits from `pydantic`'s `BaseModel`, therefore,
    a JSON representation of the data product can be generated by
    calling `model_dump_json` method on the instance of this class.

    Example:

      from npg_id_generation.pac_bio import PacBioEntity, concatenate_tags

      tags = concatenate_tags(['ACGTACGT', 'TACCCGAA'])
      o = PacBioEntity(run_name="RUN_1", well_label="A1", tags=tags)
      id = o.hash_product_id()
      # use any appropriate options of the model_dump_json method
      json_string = o.model_dump_json(exclude_none=True)
    """
    ...
    run_name: str = Field(title="PacBio run name as in LIMS")
    well_label: str = Field(title="PacBio well label")
    plate_number: Optional[int] = Field(
        default=None,
        ge=1,
        title="PacBio plate number",
        description="""
    ...
    def hash_product_id(self):
        """Returns a unique ID for the PacBio data product.

        Generates and returns a unique ID for an instance of the PacBioEntity
        class.

        To maintain backwards compatibility for PacBio Revio products,
        plate_number value of 1 is disregarded. The method should generate the
        same ID for the following two objects:
        ...
        Returns: A 64 character long string containing only hexadecimal digits.
        """
